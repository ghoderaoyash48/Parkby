...
def map_numeric_class_dataset_specific(dataset_name, original_class_id):
    ...
    Args:
        ...
        original_class_id: Original numeric class ID
        
    Returns:
        int: Mapped class ID (0: bike, 1: bus, 2: car, 3: truck) or None if unmapped
    """
    ...
    if "Vehicle_Detection" in dataset_name:
        ...
        # This is a placeholder - adjust based on actual content
        mapping = {
...
def convert_to_yolo_format(box, img_width, img_height):
    ...
    Args:
        box: Tuple (x_min, y_min, x_max, y_max) or dict with these keys
...
def process_folder(input_path, output_base, dataset_name, dataset_type):
    """
    Process a folder of data (train, val, test).
    
    Args:
        input_path: Path to input folder
        output_base: Base path for output
        dataset_name: Name of the dataset
        dataset_type: Type of data (train, val, test)
    """
...
def create_data_yaml(output_base, dataset_name):
...
def process_dataset(dataset_path, output_base, skip_existing=False):
    """
    ...
    # Create data.yaml for this dataset
    create_data_yaml(output_dir, dataset_name)
    logger.info(f"Completed processing dataset: {dataset_name}")

def main():
    parser = argparse.ArgumentParser(description='Preprocess vehicle detection datasets for YOLO format')
    parser.add_argument('--data_root', type=str, default='D:/projectfolders/ParkAI/data',
                        help='Root folder containing all datasets')
    parser.add_argument('--output_dir', type=str, default='D:/projectfolders/ParkAI/processed_data',
    ...
    for dataset_folder in dataset_folders:
        process_dataset(dataset_folder, args.output_dir, args.skip_existing)
